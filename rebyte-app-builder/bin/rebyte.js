...
var frameworks = [
  {
    ...
    ssr: {
      ...
      staticDirectory: ".open-next/assets",
      serverDirectory: ".open-next/server-functions/default",
      detectSsr: (packageJson, configContent) => {
        if (configContent) {
          return !configContent.includes("output: 'export'") && !configContent.includes('output: "export"');
        }
        return true;
      }
    }
  },
  {
    ...
    ssr: {
      buildCommand: "NITRO_PRESET=aws-lambda ./node_modules/.bin/nuxt build",
      outputDirectory: ".output",
      staticDirectory: ".output/public",
      serverDirectory: ".output/server",
      detectSsr: (packageJson, configContent) => {
        if (configContent) {
          return !configContent.includes("ssr: false");
        }
        return true;
      }
    }
  ...
  },
  {
    ...
    ssr: {
      ...
      detectSsr: (packageJson, configContent) => {
        ...
        return !deps["@sveltejs/adapter-static"];
      }
    }
  },
  {
    ...
    ssr: {
      buildCommand: "npm run build",
      outputDirectory: "dist",
      staticDirectory: "dist/client",
      serverDirectory: "dist/server",
      detectSsr: (packageJson, configContent) => {
        if (configContent) {
          return configContent.includes("output: 'server'") || configContent.includes('output: "server"');
        }
        return false;
      }
    }
...
async function detectFramework(projectDir) {
  ...
  if (!fs.existsSync(packageJsonPath)) {
    const indexHtmlPath2 = path.join(projectDir, "index.html");
    if (fs.existsSync(indexHtmlPath2)) {
      return {
        framework: staticHtmlConfig,
        isSSR: false,
        confidence: "high"
      };
    }
    throw new Error("No package.json or index.html found. " + "Make sure you are in the project root directory.");
  }
  ...
  for (const framework of frameworks) {
    ...
    if (detectors.dependencies) {
      const hasDepMatch = detectors.dependencies.some((dep) => (dep in allDeps));
      if (hasDepMatch) {
        let configFile;
        let configContent;
        if (detectors.configFiles) {
          for (const configFileName of detectors.configFiles) {
        ...
        let isSSR = false;
        if (framework.ssr) {
          isSSR = framework.ssr.detectSsr(packageJson, configContent);
        }
        return {
          framework,
          isSSR,
  ...
  throw new Error("Could not detect framework. " + "Please ensure package.json has the framework as a dependency, " + "or that the framework config file exists.");
}
function getOutputDirectory(detection) {
  if (detection.isSSR && detection.framework.ssr) {
    return detection.framework.ssr.outputDirectory;
  }
  return detection.framework.build.outputDirectory;
}
function getBuildCommand(detection) {
  return detection.framework.build.command;
}
function getSSRBuildCommand(detection) {
  if (detection.isSSR && detection.framework.ssr) {
    return detection.framework.ssr.buildCommand;
  }
  return null;
}
...
async function runCommand(command, args, cwd, verbose, extraEnv = {}) {
  return new Promise((resolve, reject) => {
...
function hasPnpmLock(projectDir) {
  return fs2.existsSync(path2.join(projectDir, "pnpm-lock.yaml"));
}
async function installDependencies(projectDir, detection, verbose) {
  if (detection.framework.slug === "nextjs" && detection.isSSR) {
    if (hasPnpmLock(projectDir)) {
      console.log("⚠️  Removing pnpm artifacts (Lambda requires npm for SSR)...");
      const pnpmLock = path2.join(projectDir, "pnpm-lock.yaml");
      const nodeModules = path2.join(projectDir, "node_modules");
      if (fs2.existsSync(pnpmLock))
        fs2.unlinkSync(pnpmLock);
      if (fs2.existsSync(nodeModules))
        fs2.rmSync(nodeModules, { recursive: true });
    }
    console.log("\uD83D\uDCE6 Installing dependencies with npm...");
    await runCommand("npm", ["install"], projectDir, verbose);
  } else {
    if (hasPnpmLock(projectDir)) {
      console.log("\uD83D\uDCE6 Installing dependencies with pnpm...");
      await runCommand("pnpm", ["install"], projectDir, verbose);
    } else {
      console.log("\uD83D\uDCE6 Installing dependencies with npm...");
      await runCommand("npm", ["install"], projectDir, verbose);
    }
...
async function runBuild(projectDir, detection, verbose) {
  const buildCommand = getBuildCommand(detection);
  if (!buildCommand) {
    console.log("ℹ️  No build command needed");
    return;
  }
  console.log(`\uD83D\uDD28 Building with: ${buildCommand}`);
  const [cmd, ...args] = buildCommand.split(" ");
  await runCommand(cmd, args, projectDir, verbose);
}
async function runSSRBuild(projectDir, detection, verbose) {
  const ssrBuildCommand = getSSRBuildCommand(detection);
  if (!ssrBuildCommand) {
    return;
  }
  console.log(`\uD83D\uDD04 Running SSR build: ${ssrBuildCommand}`);
  const parts = ssrBuildCommand.split(" ");
  const envVars = {};
  let cmdStart = 0;
...
async function build(detection, options) {
  ...
  if (!skipBuild) {
    await runBuild(projectDir, detection, verbose);
  }
  if (detection.isSSR) {
...
function addOpenNextToZip(archive, openNextDir, functionConfig) {
  ...
  if (fs3.existsSync(assetsDir)) {
    archive.directory(assetsDir, "static");
  }
...
function addSSRToZip(archive, outputDir, framework, functionConfig) {
  ...
  switch (framework) {
    case "nuxt":
      clientDir = path3.join(projectRoot, ".output", "public");
      serverDir = path3.join(projectRoot, ".output", "server");
      break;
    case "react-router":
      clientDir = path3.join(projectRoot, "build", "client");
      serverDir = path3.join(projectRoot, "build", "server");
      break;
    case "sveltekit":
      clientDir = path3.join(projectRoot, "build", "client");
      serverDir = path3.join(projectRoot, "build", "server");
      break;
    case "astro":
      clientDir = path3.join(projectRoot, "dist", "client");
      serverDir = path3.join(projectRoot, "dist", "server");
      break;
    default:
      clientDir = path3.join(outputDir, "client");
      serverDir = path3.join(outputDir, "server");
      break;
  }
  if (fs3.existsSync(clientDir)) {
    archive.directory(clientDir, "static");
  } else {
...
function addStaticToZip(archive, outputDir) {
...
async function deploy(buildResult, detection, options) {
  ...
  await uploadZip(uploadUrl, zipBuffer);
...
program.command("detect").description("Detect framework and show build configuration").option("-d, --dir <path>", "Project directory", ".").action(async (options) => {
  try {
    const projectDir = path4.resolve(options.dir);
    console.log(`
\uD83D\uDD0D Detecting framework in ${projectDir}...
`);
    const detection = await detectFramework(projectDir);
    console.log(`  Framework:    ${detection.framework.name}`);
    console.log(`  Mode:         ${detection.isSSR ? "SSR (Server-Side Rendering)" : "Static"}`);
    console.log(`  Confidence:   ${detection.confidence}`);
    if (detection.configFile) {
      console.log(`  Config:       ${detection.configFile}`);
    }
    console.log(`  Build cmd:    ${detection.framework.build.command || "(none)"}`);
    console.log(`  Output dir:   ${getOutputDirectory(detection)}`);
    if (detection.isSSR && detection.framework.ssr) {
      console.log(`  SSR build:    ${detection.framework.ssr.buildCommand}`);
    }
    console.log("");
    if (detection.framework.slug === "nextjs" && detection.isSSR) {
      console.log(`ℹ️  Next.js SSR detected. Will use OpenNext transformation for Lambda deployment.
`);
    }
  } catch (err) {
    console.error(`
...
program.command("build").description("Build the project without deploying").option("-d, --dir <path>", "Project directory", ".").option("--skip-install", "Skip dependency installation").option("-v, --verbose", "Show build output").action(async (options) => {
  try {
    const projectDir = path4.resolve(options.dir);
    console.log(`
\uD83D\uDD0D Detecting framework...
`);
    const detection = await detectFramework(projectDir);
    console.log(`  Detected: ${detection.framework.name} (${detection.isSSR ? "SSR" : "Static"})
`);
    console.log(`\uD83D\uDD28 Building project...
`);
    const result = await build(detection, {
      projectDir,
...
✅ Build complete!`);
    ...
    if (result.hasOpenNext) {
      console.log(`   Type: OpenNext (Next.js SSR)`);
    }
    console.log("");
  } catch (err) {
    console.error(`
...
program.command("deploy").description("Build and deploy the project").option("-d, --dir <path>", "Project directory", ".").option("--skip-install", "Skip dependency installation").option("--skip-build", "Skip build step (use existing build output)").option("-p, --prefix <name>", "Deployment prefix (for multiple sites per workspace)").option("-v, --verbose", "Show build output").option("--api-url <url>", "API URL (override)").option("--api-token <token>", "API token (override)").action(async (options) => {
  try {
    const projectDir = path4.resolve(options.dir);
...
`);
    ...
    const buildResult = await build(detection, {
      projectDir,
      skipInstall: options.skipInstall,
...
`);
    ...
    const result = await deploy(buildResult, detection, {
      apiUrl,
      apiToken,
...
`);
    if (detection.isSSR) {
      console.log(`ℹ️  SSR deployment: Static assets on CDN, dynamic routes via Lambda.
`);
    } else {
      console.log(`ℹ️  SSL certificate will be provisioned automatically (1-2 minutes).
`);
    }
  } catch (err) {
    console.error(`
