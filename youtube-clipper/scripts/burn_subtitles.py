...
Handle FFmpeg libass support and path space issues
"""
...
def check_libass_support(ffmpeg_path: str) -> bool:
    """
    Check if FFmpeg supports libass (required for subtitle burning)
...
def install_ffmpeg_full_guide():
    """
    Display guide for installing ffmpeg-full
    """
    print("\n" + "="*60)
    print("ffmpeg-full is required for subtitle burning")
    print("="*60)

    if platform.system() == 'Darwin':
        print("\nmacOS installation:")
        print("  brew install ffmpeg-full")
        print("\nAfter installation, FFmpeg path:")
        print("  /opt/homebrew/opt/ffmpeg-full/bin/ffmpeg  (Apple Silicon)")
        print("  /usr/local/opt/ffmpeg-full/bin/ffmpeg     (Intel)")
    else:
        print("\nOther systems:")
        print("  Please compile FFmpeg from source with libass support")
...
def burn_subtitles(
    ...
    """
    Burn subtitles into video (using temp directory to solve path space issues)
    ...
    # Create temp directory (to solve path space issues)
    temp_dir = tempfile.mkdtemp(prefix='youtube_clipper_')
    print(f"   Using temp directory: {temp_dir}")

    try:
        # Copy files to temp directory (path without spaces)
        ...
        # Use subtitles filter to burn subtitles
...
def main():
    ...
    try:
        ...
        print(f"\nComplete! Output file: {result_path}")
