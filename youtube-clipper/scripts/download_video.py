...
def download_video(url: str, output_dir: str = None) -> dict:
    ...
    ydl_opts = {
        # Video format: up to 1080p, prefer mp4
        'format': 'bestvideo[height<=1080][ext=mp4]+bestaudio[ext=m4a]/best[height<=1080][ext=mp4]/best',

        # Output template: use video ID to avoid special character issues
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            for ext in subtitle_exts:
                ...
                # Handle subtitle files with language code
                if not potential_sub.exists():
                    # Try <filename>.en.vtt format
            ...
            if subtitle_path and subtitle_path.exists():
                ...
                print(f"Warning: English subtitles not found")
                print(f"   Note: Some videos may not have subtitles or require auto-generation")
...
def _progress_hook(d):
    ...
    if d['status'] == 'downloading':
        ...
        if 'downloaded_bytes' in d and 'total_bytes' in d and d['total_bytes']:
            ...
            # Use \r for progress bar overwrite
            bar_length = 30
            ...
            # No total size info, just show downloaded
            downloaded = format_file_size(d['downloaded_bytes'])
            speed = d.get('speed', 0)
            speed_str = format_file_size(speed) + '/s' if speed else 'N/A'
...
def main():
    ...
    try:
        ...
        # Output JSON result (for use by other scripts)
        print("\n" + "="*60)
